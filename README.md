...
- Поиск законодательства **РБ** на **pravo.by** (Национальный реестр правовых актов) через чат
...
| `PRAVO_ENABLED` | Инструменты pravo.by в `/api/chat` (`pravo_search`, `pravo_get_document`). По умолчанию включены (`1`) |
...
Законодательство РБ (`pravo.by`) в `/api/chat`: модель ищет акты в Национальном реестре (`pravo_search`: название/тема или регистрационный номер вроде `2/1742`) и открывает карточку или HTML-публикацию (`pravo_get_document`). Официального API нет — читаются те же HTML-страницы, что и в браузере. Это официальное опубликование, не эталонная сводная редакция и не юридическая консультация; в ответе пользователю инструмент просит ссылаться на pravo.by. Выключить: `PRAVO_ENABLED=0`.
